"""
Field Schedule — backend API, as an application factory.

This is the same Flask + SQLite service that used to live directly in
app.py (users/locations/work types/required approvers/events over one
SQLite file), refactored so the Flask app and its SQLite connection are
built by create_app(db_path, ...) instead of constructed once at import
time from a module-level global. Nothing about the routes, validation, or
response shapes changed — this is a structural refactor, not a behavior
change.

Two callers use this:

- server/app.py — standalone, single-schedule mode (unchanged from a user
  perspective): builds exactly one app at import time from
  SCHEDULE_DB_PATH, same as before this refactor.
- server/main.py — multi-calendar / folder-tree mode: builds one of these
  per calendar, on demand, each bound to that calendar's own .db file.
  Because each call to create_app() gets its own Flask app instance with
  its own get_db() closing over its own db_path, two calendars never
  share a connection, a Flask `g`, or a lock domain — a slow write (or an
  open transaction) against Calendar A's SQLite file has no effect
  whatsoever on Calendar B's file. That isolation falls out of "these are
  just two independent Flask apps pointed at two different files" rather
  than needing any special-cased locking code.
"""
...
def seed_if_empty(db):
    ...
    (or a freshly-created calendar) doesn't open to an empty, user-less
    calendar. Safe to call repeatedly — each table is only seeded if it's
    still empty."""
...
def init_db(db_path, seed=True):
    """Brings db_path fully up to date (creating the file from nothing if
    it doesn't exist yet) and, unless seed=False, seeds starter data into
    an empty database. This is the single entry point both app.py (dev
    standalone run) and main.py (lazy per-calendar creation) call."""
...
# ------------------------------------------------------------------ app --
def create_app(db_path, static_folder=None, static_url_path="", dev_cors=False):
    """Builds one fully independent Flask app bound to db_path. Call this
    once per calendar; don't share the resulting app or its config across
    calendars — the whole point is that each one owns its own SQLite
    connection to its own file."""
    app = Flask(
        __name__,
    ...
    # Dev convenience only — see README.md. Off by default.
    if dev_cors:
        @app.after_request
    ...
    def get_db():
        if "db" not in g:
            ...
            # WAL lets readers proceed while a write is in flight against
            # THIS file. It does nothing for cross-file contention (that's
            # already solved by every calendar having its own file/connection)
            # but it meaningfully reduces "database is locked" within one
            # busy calendar.
    ...
    # --------------------------------------------------------------- SSO --
    # SAML 2.0 goes here once you have real Identity Provider metadata —
    # see README.md ("Login: SAML 2.0"). Unchanged by this refactor.
